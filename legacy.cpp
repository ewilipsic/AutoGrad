...
class Tensor{
    ...
    Tensor(std::vector<int> _shape,const std::vector<T1>& _vec,int _require_grad = 0){
        ...
        if(_require_grad) *grad = Tensor(shape,0.0,_require_grad = 0);
        out_degree = 0;
...
public:
    ...
    Tensor* grad() {return ptr->grad;}
    Node** grad_fn() {return &(ptr->grad_fn);}
    int& out_degree() const { return ptr->out_degree; }
...
class AddBackward : public Node{
    ...
    void _backward(Tensor external_grad){
        for(auto& x : operands){
            if(x.require_grad()){
                x.out_degree()--;
